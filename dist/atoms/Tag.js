...
export default function Tag({ children, className = "", variant = "cyan", removable = false, onRemove, interactive = false, }) {
    ...
    return (_jsxs("span", { role: interactive ? "button" : undefined, tabIndex: interactive ? 0 : undefined, className: cn("inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium transition-all", interactive && cn("cursor-pointer", focusRing), variant === "cyan" &&
            ...
            cn("bg-gradient-to-r from-cyan-500/10 to-purple-500/10 text-cyan-300", "border border-cyan-500/30 hover:border-cyan-400/60"), className), children: [children, removable && (_jsx("button", { type: "button", onClick: onRemove, "aria-label": "Remove tag", className: cn("ml-1 hover:opacity-75 transition-opacity cursor-pointer p-0.5 rounded-full", focusRing), children: _jsx(IconClose, {}) }))] }));
}
