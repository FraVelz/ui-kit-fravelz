...
import { cn } from "../kit";
import type { ButtonVariant, SizeSmMdLg } from "../../src/types";

type RouterButtonLinkProps = Omit<LinkProps, "className"> & {
  children: ReactNode;
  className?: string;
...
export default function RouterButtonLink({
  children,
  className = "",
  variant = "primary",
  size = "md",
  ...
  return (
    <Link
      {...props}
      className={cn(
        "inline-flex items-center justify-center gap-2 rounded-full cursor-pointer transition-all duration-300",
